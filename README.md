...
**演示视频：待录制。** [2 分钟录制脚本](docs/demo-script.md) · [截图位置说明](screenshots/README.md) · [简历描述](docs/resume.md)
...
应用截图尚未拍摄，按 [screenshots/README.md](screenshots/README.md) 保存后再嵌入本节；不使用占位图片冒充实测截图。
...
本项目通过资料处理、结构化输出和报告表达实践 AI 应用开发。Microsoft 两门课程可作为后续学习资源，不代表已经完成课程或实现其中全部内容。
